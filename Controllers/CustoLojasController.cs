...
namespace RelatoriosRosset.Controllers
{
    public class CustoLojasController : Controller
    {
        ...
        public CustoLojasController(ApplicationDbContext context, IConfiguration configuration)
        {
            _context = context;
            _timeoutSegundos = configuration.GetValue<int?>("Relatorios:CustoLojasTimeoutSegundos")
                               ?? SqlComandoExtensions.TimeoutPadraoSegundos;
        }
        ...
        public async Task<IActionResult> ExecutarProcedure(DateTime dataInicial, DateTime dataFinal, string filial)
        {
            ...
            try
            {
                await _context.ExecutarProcedureAsync(
                    "EXEC GERA_CUSTO_LOJAS_EAN @DataInicial, @DataFinal, @Filial",
                    cmd => cmd
                        .ComParametro("@DataInicial", SqlDbType.Date, dataInicial)
                        .ComParametro("@DataFinal", SqlDbType.Date, dataFinal)
                        .ComParametro("@Filial", SqlDbType.NVarChar, filial),
                    timeoutSegundos: _timeoutSegundos);
            ...
            {
                model.Mensagem = $"Erro ao executar a procedure: {ex.Message}";
                Console.WriteLine($"Erro ao executar a procedure: {ex.Message}\n{ex.StackTrace}");
            }
        ...
        public async Task<IActionResult> GerarRelatorio()
        {
            try
            {
                ...
                using var connection = await _context.AbrirConexaoAsync();
                using var command = connection.CriarComando(@"
                    SELECT
                        TRY_CAST(ITEM AS INT) AS Item,
                        DESC_ITEM_COMPOSICAO AS DescItemComposicao,
                        ITEM_COMPOSICAO AS ItemComposicao,
                    ...
                    FROM TABELA_CUSTO_EAN
                    WHERE TRY_CAST(ITEM AS INT) IS NOT NULL
                    ORDER BY PRODUTO", _timeoutSegundos);
                ...
                for (int i = 0; i < resultados.Count; i++)
                {
                    ...
                    worksheet.Cell(i + 2, 7).Value = resultados[i].CodigoBarra;
                    worksheet.Cell(i + 2, 8).Value = resultados[i].Qtde;
                    worksheet.Cell(i + 2, 9).Value = resultados[i].ValorCusto;
                    worksheet.Cell(i + 2, 10).Value = resultados[i].Doc;
                    worksheet.Cell(i + 2, 11).Value = resultados[i].RomaneioProduto;
                    worksheet.Cell(i + 2, 12).Value = resultados[i].SerieNf;
                    worksheet.Cell(i + 2, 13).Value = resultados[i].Cfop;
                    ...
                    worksheet.Cell(i + 2, 17).Value = resultados[i].DataMov == DateTime.MinValue ? "" : resultados[i].DataMov.ToString("dd/MM/yyyy");
                    worksheet.Cell(i + 2, 18).Value = resultados[i].TotalQtde;
                    worksheet.Cell(i + 2, 19).Value = resultados[i].ValorTotal;
                    worksheet.Cell(i + 2, 20).Value = resultados[i].ValorImpostoDestacar;
                    worksheet.Cell(i + 2, 21).Value = resultados[i].ValorLiq;
                    worksheet.Cell(i + 2, 22).Value = resultados[i].ValorProducao;
                    worksheet.Cell(i + 2, 23).Value = resultados[i].ValorBruto;
                    worksheet.Cell(i + 2, 24).Value = resultados[i].VlIcmsOp;
                }
                ...
                worksheet.Columns(1, 24).AdjustToContents();
                worksheet.Row(1).Style.Font.Bold = true;
