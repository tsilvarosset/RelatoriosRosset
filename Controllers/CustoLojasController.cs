    ...
    public class CustoLojasController : Controller
    {
        ...
        public async Task<IActionResult> GerarRelatorio()
        {
            try
            {
                ...
                worksheet.Cell(1, 6).Value = "Código de Barra";
                worksheet.Cell(1, 7).Value = "Quantidade";
                worksheet.Cell(1, 8).Value = "Valor Custo";
                worksheet.Cell(1, 9).Value = "Documento";
                worksheet.Cell(1, 10).Value = "Romaneio Produto";
                worksheet.Cell(1, 11).Value = "Série NF";
                worksheet.Cell(1, 12).Value = "CFOP";
                worksheet.Cell(1, 13).Value = "Descrição CFOP";
                worksheet.Cell(1, 14).Value = "Filial";
                worksheet.Cell(1, 15).Value = "Rateio Filial";
                worksheet.Cell(1, 16).Value = "Data Movimento";
                worksheet.Cell(1, 17).Value = "Total Quantidade";
                worksheet.Cell(1, 18).Value = "Valor Total";
                worksheet.Cell(1, 19).Value = "Valor Imposto Destacar";
                worksheet.Cell(1, 20).Value = "Valor Líquido";
                worksheet.Cell(1, 21).Value = "Valor Produção";
                worksheet.Cell(1, 22).Value = "Valor Bruto";
                worksheet.Cell(1, 23).Value = "Valor ICMS Operação";
                ...
                for (int i = 0; i < resultados.Count; i++)
                {
                    ...
                    worksheet.Cell(i + 2, 6).Value = resultados[i].CodigoBarra;
                    worksheet.Cell(i + 2, 7).Value = resultados[i].Qtde;
                    worksheet.Cell(i + 2, 8).Value = resultados[i].ValorCusto; // Nullable
                    worksheet.Cell(i + 2, 9).Value = resultados[i].Doc;
                    worksheet.Cell(i + 2, 10).Value = resultados[i].RomaneioProduto;
                    worksheet.Cell(i + 2, 11).Value = resultados[i].SerieNf;
                    worksheet.Cell(i + 2, 12).Value = resultados[i].Cfop;
                    worksheet.Cell(i + 2, 13).Value = resultados[i].DescricaoCfop;
                    worksheet.Cell(i + 2, 14).Value = resultados[i].Filial;
                    worksheet.Cell(i + 2, 15).Value = resultados[i].RateioFilial;
                    worksheet.Cell(i + 2, 16).Value = resultados[i].DataMov == DateTime.MinValue ? "" : resultados[i].DataMov.ToString("dd/MM/yyyy");
                    worksheet.Cell(i + 2, 17).Value = resultados[i].TotalQtde;
                    worksheet.Cell(i + 2, 18).Value = resultados[i].ValorTotal; // Nullable
                    worksheet.Cell(i + 2, 19).Value = resultados[i].ValorImpostoDestacar; // Nullable
                    worksheet.Cell(i + 2, 20).Value = resultados[i].ValorLiq; // Nullable
                    worksheet.Cell(i + 2, 21).Value = resultados[i].ValorProducao; // Nullable
                    worksheet.Cell(i + 2, 22).Value = resultados[i].ValorBruto; // Nullable
                    worksheet.Cell(i + 2, 23).Value = resultados[i].VlIcmsOp; // Nullable
                }

                // Apply number formatting
                worksheet.Column(1).Style.NumberFormat.Format = "0"; // Item
                worksheet.Column(3).Style.NumberFormat.Format = "0"; // ItemComposicao
                worksheet.Column(7).Style.NumberFormat.Format = "0"; // Qtde
                worksheet.Column(17).Style.NumberFormat.Format = "0"; // TotalQtde
                worksheet.Column(8).Style.NumberFormat.Format = "#,##0.00"; // ValorCusto
                worksheet.Column(18).Style.NumberFormat.Format = "#,##0.00"; // ValorTotal
                worksheet.Column(19).Style.NumberFormat.Format = "#,##0.00"; // ValorImpostoDestacar
                worksheet.Column(20).Style.NumberFormat.Format = "#,##0.00"; // ValorLiq
                worksheet.Column(21).Style.NumberFormat.Format = "#,##0.00"; // ValorProducao
                worksheet.Column(22).Style.NumberFormat.Format = "#,##0.00"; // ValorBruto
                worksheet.Column(23).Style.NumberFormat.Format = "#,##0.00"; // VlIcmsOp
