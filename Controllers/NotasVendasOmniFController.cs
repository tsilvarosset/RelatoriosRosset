...
{
    public class NotasVendasOmniFController : Controller
    {
        private readonly ApplicationDbContext _context;

        public NotasVendasOmniFController(ApplicationDbContext context)
        {
            _context = context;
        }
        public async Task<IActionResult> NotasVendasOmniF(DateTime? dataInicio, DateTime? dataFim, string clienteVarejo, string cpfCgc)
        {
            ...
            if (!dataInicio.HasValue && !dataFim.HasValue && string.IsNullOrEmpty(clienteVarejo) && string.IsNullOrEmpty(cpfCgc))
            {
                ...
                return View(new List<NotasVendasOmniFModel>());
            }

            var query = _context.NOTAS_VENDAS_FRANQUIAS.AsQueryable();
        ...
        public async Task<IActionResult> ExportarExcel(DateTime? dataInicio, DateTime? dataFim, string clienteVarejo, string cpfCgc)
        {
            try
            {
                ...
                if (!notasF.Any())
                {
                    return RedirectToAction(nameof(NotasVendasOmniF), new { dataInicio, dataFim, clienteVarejo, cpfCgc });
                }
