...
{
    public class VendasOmniPController : Controller
    {
            private readonly ApplicationDbContext _context;

            public VendasOmniPController(ApplicationDbContext context)
            {
                _context = context;
            }

            
            public async Task<IActionResult> VendasOmniP(DateTime? dataInicio, DateTime? dataFim)
            {
                var query = _context.V_VENDAS_OMNI_P.AsQueryable();
            ...
            public async Task<IActionResult> ExportarExcel(DateTime? dataInicio, DateTime? dataFim)
            {
                try
                {
                    ...
                    if (!lojaVendas.Any())
                    {
                        return RedirectToAction(nameof(VendasOmniP), new { dataInicio, dataFim });
                    }
