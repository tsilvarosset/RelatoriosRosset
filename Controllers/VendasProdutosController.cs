...
namespace RelatoriosRosset.Controllers
{
    public class VendasProdutosController : Controller
    {
        ...
        public async Task<IActionResult> VendasProdutos(DateTime? dataInicio, DateTime? dataFim)
        {
            if (!dataInicio.HasValue && !dataFim.HasValue)
            {
                ViewBag.DataInicio = null;
                ViewBag.DataFim = null;
                return View(new List<VendasProdutosModel>()); // Return empty list to avoid null Model
            }

            var query = _context.V_VENDAS_PRODUTOS_P.AsQueryable();
        ...
        public async Task<IActionResult> ExportarExcel(DateTime? dataInicio, DateTime? dataFim)
        {
            try
            {
                ...
                if (!lojaVendas.Any())
                {
                    return RedirectToAction(nameof(VendasProdutos), new { dataInicio, dataFim });
                }
